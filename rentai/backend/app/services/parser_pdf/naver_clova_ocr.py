...
def extract_text_from_image_clova(image_bytes: bytes, format: str = "jpg") -> Dict[str, Any]:
    ...
    # 지원 형식: jpg, jpeg, png
    if format_lower not in ['jpg', 'jpeg', 'png']:
        # 기본값으로 jpg 사용
        format_lower = 'jpg'
        logger.warning(f"[Naver Clova OCR] 지원하지 않는 형식 '{format}', 'jpg'로 변경")
    
    # 이미지 크기 검증 (너무 크면 리사이즈)
    try:
        ...
        # Clova OCR은 최대 4096x4096 픽셀 권장
        max_size = 4096
        if width > max_size or height > max_size:
            logger.info(f"[Naver Clova OCR] 이미지 크기 조정: {width}x{height} -> 최대 {max_size}px")
        ...
        logger.warning(f"[Naver Clova OCR] 이미지 검증 실패, 원본 사용: {e}")
    ...
    files = [
        ('file', ('image.' + format_lower, image_bytes, f'image/{format_lower}'))
    ]
    headers = {
    ...
    try:
        logger.info(f"[Naver Clova OCR] API 호출 중... (형식: {format_lower}, 크기: {len(image_bytes)} bytes)")
        response = requests.post(
            NAVER_CLOVA_API_URL,
            headers=headers,
            data=payload,
            files=files,
            timeout=60  # 타임아웃을 60초로 증가 (이미지 마스킹 처리 시 충분한 시간 확보)
        )
        ...
        if response.status_code == 400:
            try:
                ...
                logger.error(f"[Naver Clova OCR] 400 에러 상세: {error_detail}")
            except:
                logger.error(f"[Naver Clova OCR] 400 에러 응답: {response.text[:500]}")
